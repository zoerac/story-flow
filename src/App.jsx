  ...
  return (
    <div style={{ minWidth: 0 }}>
      ...
      <div style={S.visualGridTwo}>
        {items.map((item, i) => (
          <VisualCandidateCard key={item.id} item={item} active={item.id === selectedId} best={bestIds.has(item.id)} onSelect={() => onSelect(item.id)} index={i} />
        ))}
      </div>
    </div>
  );
...
function VisualCandidateCard({ item, active, best, onSelect, index = 0 }) {
  return (
    <button
      type="button"
      onClick={onSelect}
